...
export default function DomainDetail() {
  ...
  const related = domains
    .filter((d) => d.slug !== domain.slug && d.categories.some((c) => domain.categories.includes(c)))
    .slice(0, 4);
  ...
  return (
    <>
      ...
      <div className="container" style={{ padding: 'var(--space-5) 0' }}>
        ...
        <p style={{ marginBottom: 'var(--space-3)', fontSize: 'var(--font-size-h3)', color: 'var(--color-accent-secondary)' }}>
          {domain.thesis}
        </p>
        <p>
          Status: <strong>{domain.status}</strong> | Price: <strong>{priceDisplay}</strong>
        </p>
        ...
        <section style={{ marginTop: 'var(--space-4)' }}>
          ...
          <p>
            All transactions complete via independent escrow to ensure security for both buyer and seller. Domain
            transfer is initiated only after funds are secured in escrow and typically completes within 24–48 hours.
          </p>
        </section>
